...
"""
    ...
    def generate_review_article(self, item: Dict[str, Any]) -> str:
        ...
        prompt = f"""以下の楽天Kobo電子書籍の商品情報を元に、はてなブログの「編集部おすすめ」に選出されるレベルの、熱量が高く独自の視点に満ちた個人ブログのレビュー記事を執筆してください。
...
- 「独自のこだわり評価軸」を必ず1つ設定して語ってください（例: 「この作品がもたらす極上の読書体験」「仕事の合間に脳をリセットする瞬間」など）。
- デメリットや「ここは惜しい！」というポイントを、スペック批判ではなく、読後のリアルな感想・本音として包み隠さず書いてください。
...
4. 商品リンクは, 文末付近に自然な形で `[楽天Koboで「{clean_title}」の電子書籍をチェックする]({url})` のようにMarkdownのリンク記法で埋め込んでください。
